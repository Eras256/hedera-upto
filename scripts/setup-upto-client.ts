...
// Once the mirror node has resolved the alias to a real account id (e.g.
// after the faucet's initial funding transfer already touched it), pass
// it here to skip alias-form resolution -- some consensus nodes reject
// an alias-form transactionId as payer at precheck (PAYER_ACCOUNT_NOT_FOUND)
// even though the account genuinely exists and is queryable by alias.
const KNOWN_ACCOUNT_ID = process.env.UPTO_CLIENT_KNOWN_ACCOUNT_ID;
...
async function main() {
  ...
  // Step 1: finalize the hollow account with a trivial self-paid
  // transaction, using the real account id if already known (see note
  // above on PAYER_ACCOUNT_NOT_FOUND with the alias-form transactionId).
  const payerAccountId = KNOWN_ACCOUNT_ID
    ? AccountId.fromString(KNOWN_ACCOUNT_ID)
    : AccountId.fromEvmAddress(0, 0, CLIENT_EVM!.replace(/^0x/, ""));
  client.setOperator(payerAccountId, key);

  console.log("Finalizando cuenta hollow con un self-transfer de 1 tinybar...");
  const finalizeTx = await new TransferTransaction()
    .addHbarTransfer(payerAccountId, Hbar.fromTinybars(-1))
    .addHbarTransfer(payerAccountId, Hbar.fromTinybars(1))
    .execute(client);
  const finalizeReceipt = await finalizeTx.getReceipt(client);
  console.log("Status:", finalizeReceipt.status.toString());

  const realAccountId = payerAccountId;
  console.log("Cuenta real:", realAccountId.toString());
